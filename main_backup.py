from flask import Flask, render_template, request, jsonify, redirect, url_for, flash, make_response, Response
...
virtual_state = {
    'anomaly': 'none',
    'auto_mode': True,  # Enable automatic anomaly injection
    ...
    while True:
        if virtual_state['simulation_running']:
            try:
                ...
                # Auto-inject anomalies occasionally
                if virtual_state['auto_mode'] and random.random() < 0.02:  # 2% chance every 2 seconds
...
def generate_sensor_data():
    ...
    pressure = random.uniform(95, 105)  # New sensor
    rpm = random.uniform(2800, 3200)    # New sensor

    # Apply manual triggers or keep random subtle anomalies
    if state == 'heat':
    ...
    elif random.random() < 0.05: # Reduced random failure chance
    ...
    if model:
        # Prepare for model (needs 2D array) - using only temp, vib, curr for now
        features = np.array([[data['temperature'], data['vibration'], data['current']]])
        pred = model.predict(features)[0]
        probs = model.predict_proba(features)[0]
        
        # Risk is the probability of class 1 (Failure)
        ...
        # Determine Root Cause if failure
...
class User(UserMixin, db.Model):
    ...
    password = db.Column(db.String(150), nullable=False) # In a real app, hash this!
...
class SensorLog(db.Model):
    ...
    pressure = db.Column(db.Float, nullable=True)  # New sensor
    rpm = db.Column(db.Float, nullable=True)       # New sensor
...
def ensure_sqlite_columns():
    """Ensure missing SQLite columns are added when the DB already exists."""
...
def get_dashboard_metrics():
    ...
    uptime = format_duration(datetime.utcnow() - app_start_time)
...
def analytics_summary():
    ...
    # Confusion matrix values (simulated from actual vs predicted)
...
def simulate_data():
    ...
    Uses Virtual State to force extreme failures for mentor presentations.
    """
    data = generate_sensor_data()
...
@login_required
def export_csv():
    # Query all logs for the current user
    ...
    def generate():
        ...
        # Write header
        writer.writerow(('Timestamp', 'Temperature (C)', 'Vibration (mm/s)', 'Power Load (A)', 'Pressure (psi)', 'RPM', 'Prediction', 'Confidence (%)'))
        yield data.getvalue()
        data.seek(0)
        data.truncate(0)
